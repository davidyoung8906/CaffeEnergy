...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
	...
	for (int i = start; i <= end; ++i) {
		...
		if(!isWarmup)
		{
			...
			if(i > 0)
			{
				...
				//current_configuration[i] = schedule(_slack, deadlines[i],deadlines[i-1], (float)exec_us/iteration + sched_us, _Uncertainty[i],current_configuration[i-1],current_configuration[i], history[i-1], rev_history[i-1], history[i], rev_history[i]);
				current_configuration[i] = schedule(_slack, deadlines[i],deadlines[i-1], (float)exec_us/iteration + sched_us, _Uncertainty[i],current_configuration[i-1],current_configuration[i], history[i-1], rev_history[i-1], history[i], rev_history[i]);
				...
				//std::cout << "++++ Overhead: " << sched_us << " "<< (end_total-start_total)/1000000.0 << std::endl;
				total_overhead += sched_us;
	...
	std::cout << "++++ TotalOverhead: " << total_overhead << std::endl;

...
void InnerProductLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	//std::cerr << "[" << timestamp << "] fc layer" << std::endl;
	//usleep(10000);
	...
	//std::cout << "M_/bias_term: "<< M_ <<" "<< bias_term_ <<std::endl; //<<1,1>>
	if (M_ == 1) {
...
#ifdef LAYER_STATISTIC
		//uint64_t compute_times = N_ * K_ + N_  ;
		//std::cout << "\tCompute: "<< compute_times << std::endl;
#endif
	} else {
